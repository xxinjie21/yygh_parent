...
import com.yygh.hospital.util.BeanUtils;
...
public class ApiServiceImpl implements ApiService {
    ...
    public boolean removeDepartment(String depcode) {
        ...
        System.out.println("111111111111111:"+respone.toJSONString());
        log.info(String.valueOf(respone.getIntValue("code")));
        if(null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
            throw new YyghException(respone.getString("message"), 201);
    ...
    public static byte[] getImageBytes(String imgUrl) {
        ...
        try {
            ...
            throw new YyghException("图片URL格式错误", 201);
        } catch (ConnectException e) {
            log.error("获取图片时连接异常，url:{}", imgUrl, e);
            throw new YyghException("获取图片连接失败", 201);
        } catch (IOException e) {
            log.error("读取图片时发生异常，url:{}", imgUrl, e);
            throw new YyghException("读取图片失败", 201);
        } finally {
            if (is != null) {
                try {
                    is.close();
