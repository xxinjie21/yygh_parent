...
import com.yygh.order.utils.ConstantPropertiesUtils;
import com.yygh.order.utils.HttpClient;
...
public class WeixinServiceImpl implements WeixinService {
    ...
    public Boolean refund(Long orderId) {
        try {
            ...
            //判断返回的值
            if(refundInfo.getRefundStatus().intValue()== RefundStatusEnum.REFUND.getStatus().intValue()) return true; //已退款
            //未退款 调用微信接口 封装请求参数
            ...
            String xml = client.getContent();
            Map<String, String> resultMap = WXPayUtil.xmlToMap(xml);
            log.info("封装返回结果合集："+resultMap.toString());
