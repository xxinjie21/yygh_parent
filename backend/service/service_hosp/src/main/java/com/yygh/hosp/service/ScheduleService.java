...
public interface ScheduleService {
    // 上传排班接口
    void save(ScheduleSaveDTO scheduleSaveDTO);

    // 查询排班接口（分页）
    IPage<Schedule> findPageSchedule(ScheduleQueryDTO scheduleQueryDTO);

    // 删除排班接口
    void remove(String hoscode, String hosScheduleId);

    // 根据医院编号和科室编号，查询排班规则数据
    Map<String, Object> getRuleSchedule(ScheduleQueryDTO scheduleQueryDTO);

    // 根据医院编号、科室编号和工作日期查询排班详细信息
    List<ScheduleVo> getDetailSchedule(String hoscode, String depcode, String workDate);

    // 获取可预约排班数据
    Object getBookingScheduleRule(ScheduleQueryDTO scheduleQueryDTO);

    // 根据排班id获取排班数据
    ScheduleVo getScheduleId(String scheduleId);
